...
for ((i = 1 ; i <= reps ; i++)); do
	...
	for ((N = Ninicio, j = 1 ; N <= Nfinal ; N += Npaso, j++)); do
		...
		echo "Mult Tras N: $N / $Nfinal..."
		traspuesta=$(./multiplica_matriz $N | grep 'Traspuesta' | awk '{print $2}')
		x=${oldT[$j]}
		oldT[$j]=$(python -c "print( $traspuesta + $x )")
...
for ((N = Ninicio, j = 1 ; N <= Nfinal ; N += Npaso, j++)); do
	...
	normalR=$(python -c "print(${D1mrN[$j]} / $reps)")
	normalW=$(python -c "print(${D1mwN[$j]} / $reps)")
	trasR=$(python -c "print(${D1mrT[$j]} / $reps)")
	trasW=$(python -c "print(${D1mwT[$j]} / $reps)")
