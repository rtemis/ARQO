...
# Set series to 0
for ((i = 1, N = Np ; N < Nmax ; N += Npaso, i++)); do
	pfin[$i]=0
done
...
for ((i = 1 ; i <= iteration ; i++)); do
    echo "Iteration: $i"
	# Loop for sum$j
	for ((j = 1, N = Np ; N < Nmax ; N += Npaso, j++)); do
		echo "Scale $N"
...
for ((j = 1 ; j <= 4 ; j++)); do
	...
	for ((i = 1 ; i <= iteration ; i++));do
		...
		for ((k = 1, N = Np ; N < Nmax ; k++, N += Npaso)); do
			echo "N Size: $N"
			p1=$(./multiply_par1 $N $j | grep 'time' | awk {'print $2'})
			echo "Time 1: $p1"
			pp1[$k]=$(python -c "print(${pp1[$k]} + $p1)")
			p2=$(./multiply_par2 $N $j | grep 'time' | awk {'print $2'})
			echo "Time 2: $p2"
			pp2[$k]=$(python -c "print(${pp2[$k]} + $p2)")
			p3=$(./multiply_par3 $N $j | grep 'time' | awk {'print $2'})
			echo "Time 3: $p3"
